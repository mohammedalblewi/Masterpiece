...
if(isset($_POST['upd_general']))
{
    ...
    $q = "UPDATE `settingss` SET `site_title`=?, `site_about`=? WHERE `sr_no`=?";
    $values = [$frm_data['site_title'],$frm_data['site_title'],1];
...
if(isset($_POST['upd_shutdown']))
{
    ...
    $q = "UPDATE `settingss` SET `shutdown`=? WHERE `sr_no`=?";
    $values = [$frm_data,1];
...
{
    $frm_data = filteration($_POST);
    $image_r = uploadImage($_FILES['picture'],ABOUT_FOLDER);
